...
- Generate against published Hermes releases, by version, from `spec/refs.yaml`. Do not track development branches or pre-releases.
...
- Before a PR, run `swift build && swift test`, `kotlin/gradlew check`, `make gen REF=<version>`, `make live REF=<version>`, and `make coverage REF=<version>` when those targets are available for its phase.
