...
def test_tagged_rest_symbols_are_generated() -> None:
    ...
    assert operations[3].request_type_name == "ProfilesSetActiveRequest"
    assert [(p.wire, p.name, p.swift_type, p.kotlin_type) for p in operations[3].request_properties] == [
        ("name", "name", "String", "String"),
    ]
    assert [(p.wire, p.name, p.swift_type, p.kotlin_type) for p in operations[4].query_params] == [
