...
def test_tagged_rest_symbols_are_generated() -> None:
    ...
    assert [(op.namespace, op.name, op.type_name) for op in operations] == [
        ...
        ("profiles", "setActive", "ProfilesSetActiveResponse"),
        ("sessions", "emptyCount", "SessionsEmptyCountResponse"),
    ]
    assert next(p for p in operations[0].properties if p.wire == "reason").nullable
    assert operations[4].request_type_name == "ProfilesSetActiveRequest"
    assert [(p.wire, p.name, p.swift_type, p.kotlin_type) for p in operations[4].request_properties] == [
        ("name", "name", "String", "String"),
    ]
    assert [(p.wire, p.name, p.swift_type, p.kotlin_type) for p in operations[5].query_params] == [
